...
UserCollection.prototype.dialog_key_for_online_users = function (first, second) {
    first = parseInt(first, 10);
    second = parseInt(second, 10);

    if (!first || !second) {
        throw new PublicError('Messages: one of arguments is not integer.');
    }

    var current_keys = Object.keys(this.all).map(utils.parseDecimal);
    var opposite_keys = Object.keys(this.opposite.all).map(utils.parseDecimal);

    var current = (current_keys.indexOf(parseInt(first)) >= 0)
        ? first
        : (opposite_keys.indexOf(parseInt(second)) >= 0) ? second : null;

    var opposite = (opposite_keys.indexOf(parseInt(second)) >= 0)
        ? second
        : (current_keys.indexOf(parseInt(first)) >= 0) ? first : null;

    if (current === null) {
        throw PublicError("Error while calculating dialog key: can't determine current key.")
    }

    if (opposite === null) {
        throw PublicError("Error while calculating dialog key: can't determine opposite key.")
    }

    var man = this.is_man() ? current : opposite;
    var woman = this.is_woman() ? current : opposite;

    return "dialogs:" + man + '_' + woman;
};
...
UserCollection.prototype.deliver_message = function (message, sender_id) {
    ...
    key = this.dialog_key_for_online_users(sender_id, recipient_id);

    dialogs.get(key).continue_as(this.role)
        .then(function () {
    ...
    if (this.opposite.ids().indexOf(recipient_id) < 0 || (this.opposite.all[recipient_id] && this.opposite.all[recipient_id].length === 0)) {
