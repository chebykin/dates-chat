        ...
        .then(function () {
            _user_collection.for_user_profiles([message.sender_id], function (profiles) {
                utils.jsonParse(profiles[Object.keys(profiles)[0]], function(err, profile) {
                    ...
                    redis.hmget('chat_settings:' + ws.user_id, 'notifications', function (err, obj) {
                        if (err) {
                            deferred.reject(err);
                        }
                        ...
                        switch (obj[0]) {
                            ...
                            case 'native':
                                _user_collection.opposite
                                    .respond_to_ids_with([message.recipient_id], 'messages_push', 'page', message, true);
                                break;
                            default:
                                _user_collection.opposite
                                    .respond_to_ids_with([message.recipient_id], 'messages_push', 'page', message);
                        }
                    });
                });
            });
