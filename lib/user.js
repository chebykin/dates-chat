    ...
    dialogs.between(message.sender_id, message.recipient_id).deliver(message)
        .then(function () {
            _user_collection.for_user_profiles([message.sender_id], function (profiles) {
                var profile = JSON.parse(profiles[Object.keys(profiles)[0]]);

                message.name = profile.n;
                message.avatar = profile.cp;
                message.key = hat();
...
UserCollection.prototype.update_dialog_state = function (state, user_id) {
    this.respond_to_ids_with([user_id], 'dialogs_replace', 'chat', {state: state});
};
