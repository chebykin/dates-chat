...
Responder.respond = {
    ...
    to_collection: function (collection) {
        var response = new Response();

        response.type = 'collection';
        response.collection = collection;
        response.sockets = response.collection.all_websockets();
...
Response.prototype.using = function (responder) {
    if (typeof responder === 'function') {
        this.action = responder;
    } else if (typeof responder === 'string') {
        var arr = responder.split('#'),
            deferred = Q.defer();
...
Response.prototype.exec = function () {
    ...
    switch (this.type) {
        ...
        case 'users':
            ...
            break;
        case 'collection':
            response_action = this.action(this.collection);
            break;
    }
