...
var events = require('events'),
    ...
    dialogs_controller = require('../controllers/dialogs_controller').response,
    recent_users = require('../models/recent_users'),
...
function Dialog (one, another) {
    ...
    Object.defineProperty(this, 'state', {
        ...
        set: function (newValue) {
            ...
            dialogs_controller.update(state, this.man_id, this.woman_id);
            dialogs_controller.update(state, this.woman_id, this.man_id);

            console.log('Dialog ' + this.man_id + '/' + this.woman_id + ' changed state to ' + newValue.toUpperCase());
        }.bind(this)
    });
}
 ...
 */
Dialog.prototype.close = function (message, new_state) {
    message = message || 'Billing error';
    this.tracker.destruct();
    this.tracker = null;
    this.state = new_state || 'off';
    dialogs.del(this.dialog_key, message);
    console.log('Dialog ' + this.man_id + '/' + this.woman_id + ' CLOSED');
};
...
Dialog.prototype.save_message = function (message) {
    ...
    redis.expire(this.key(), config.expirations.dialog);
    recent_users.add(message.sender_id, message.recipient_id);
    redis.sadd('chat_contacts:' + message.sender_id, message.recipient_id);
    redis.sadd('chat_dialogs:' + message.sender_id, message.recipient_id);
    redis.sadd('chat_dialogs:' + message.recipient_id, message.sender_id);
};
