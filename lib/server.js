...
function actionGetMessages(ws) {
    currentCollection(ws).get_messages(ws.user_id);
}

function actionMessage(ws, data) {
    oppositeCollection(ws).deliver_message(data, ws.user_id);
}
...
function oppositeCollection(ws) {
    if (ws.role === 'man') {
        ...
        throw new Error("Can't determine opposite role");
    }
}
...
function currentCollection(ws) {
    if (ws.role === 'man') {
        ...
        throw new Error("Can't determine current role");
    }
}
